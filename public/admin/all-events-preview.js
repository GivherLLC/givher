var EventPreview = createClass({
  render: function () {
    ...
    var detailsAvailable = entry.getIn(['data', 'available']);
      ...
      var detailImage = detailImageRaw ? this.props.getAsset(detailImageRaw) : null; // Only resolve the image if it's not null/undefined
      ...
      // Validation Section (if there are errors)
        ...
        h(
          ...
          validationFields.map((item, index) => 
            h('p', { key: index }, 
              item.isValid 
                ? h('span', { style: { color: 'green', marginRight: '10px' } }, `✔️ ${item.label} is valid`) 
                : h('span', { style: { color: 'red', marginRight: '10px' } }, `❌ ${item.label} is required`)
            )
          )
        ):
        h(
