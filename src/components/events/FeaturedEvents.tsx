  ...
  return (
    <div className="bg-softOpal dark:bg-navySmoke pt-[2.5rem] md:py-[2.5rem] flex justify-center relative md:max-h-[480px] overflow-hidden">
      <div className="relative w-full h-full flex flex-col items-center justify-center gap-[2.5rem] max-w-[85.75rem] mx-[0.625rem] lg:mx-[1.5625rem]">
        <div
          id="carousel-inner"
          className="w-full h-full grid grid-cols-1 grid-rows-1 md:grid-cols-[1fr,minmax(auto,675px)] items-center justify-center md:justify-between gap-[1rem] md:gap-[50px] ml-[calc(max(0px, (100% - 1440px) / 2))]"
        >
          <div
            id="carousel-content-container"
            className="order-2 md:order-1 h-full grid grid-rows-[1fr,52px] gap-[2rem] z-10 min-w-[350px]"
          >
            <div
              id="carousel-content-inner"
            ...
            >
              {featuredEvents.map((e, i) => {
                return (
                  <div
                    key={i}
                    className={`carousel-content relative flex flex-col md:gap-[2rem] items-start transform translate-y-[50px] ${activeIndex === i ? 'active transition-all duration-500 transform translate-y-0' : ''}`}
                  >
                    <div>
                    ...
                    {e.eventButtonTextOne && e.eventButtonLinkOne && (
                      <EventButton
                        text={e.eventButtonTextOne}
                        link={e.eventButtonLinkOne ? e.eventButtonLinkOne : ''}
                        bg="electricYellow"
                        openNewTab={
                          e.eventButtonLinkOne === '#all-events' ? false : true
                        }
                      />
                    )}
                    {!e.eventButtonTextOne && (
                      <EventButton
                        text={
                          e.available ? 'Inside the Event' : 'Get Email Updates'
                        }
                        link={
                          e.available
                            ? `/events/detail/${e.slug}`
                            : '/signup4emails'
                        }
                        bg="electricYellow"
                      />
          ...
          <div className="carousel featured-carousel order-1 md:order-2 relative h-full max-h-[350px] md:max-h-[unset] overflow-hidden md:overflow-visible">
            <Image
              ...
              height={435}
              width={595}
              className="hidden md:block md:absolute left-[-30%] bottom-[-15%] z-0"
            />
