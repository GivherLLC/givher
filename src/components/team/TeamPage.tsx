            ...
            <div className="relative flex flex-col w-full items-center justify-center gap-[2.5rem] max-w-[85.75rem] px-[0.625rem] md:px-[1.5625rem]">
                ...
                <div className={`w-full flex flex-col flex-wrap xl:flex-row gap-[5rem] ${teamMembers.length > 2 ? "justify-between":"justify-around"}`}>
                    ...
                    <div key={t.name} data-id={t.name} className="xl:max-w-[25%] flex flex-col md:flex-row xl:flex-col justify-center items-center gap-[2rem] xl:gap-[1rem] w-full">
                        <img loading="eager" onClick={()=>{setShowBio(t)}} src={t.imageSrc} height={420} width={420} alt="Jay Franco headshot" className="w-full max-w-[400px] mb-[1rem] cursor-pointer"/>
                            ...
                            <div data-id="jay-icons" className="flex gap-[2rem] mb-[1rem]">
                                {t.iconLinks.map((i)=>(
                                    <Link key={i.iconImageAlt} href={i.iconLink}>
                                        <img loading="eager" src={i.iconImageSrc} alt={i.iconImageAlt} height={27} width={27} className="dark:hidden hover:scale-[1.1] transition-transform ease-in-out"/>
                                        <img loading="eager" src={i.darkmodeSrc} alt={i.iconImageAlt} height={27} width={27} className="hidden dark:block hover:scale-[1.1] transition-transform ease-in-out"/>
                                    </Link>
                                ))}
                            ...
                            <div className="bg-softOpal dark:bg-navySmoke opacity-100 max-h-[80vh] max-w-[1040px] min-h-[100px] top-[10%] mx-auto relative p-[2.5rem] overflow-y-scroll">
                                ...
                                <div className="flex flex-col md:flex-row gap-[1.5rem]">
                                    <div className="flex flex-col items-center gap-[1rem]">
                                        <img loading="lazy" src={showBio.imageSrc} height={420} width={420} alt="Jay Franco headshot" className="max-w-[325px] mb-[1rem] cursor-pointer"/>
                                        ...
                                        <div data-id="jay-icons" className="flex gap-[2rem]">
                                            {showBio.iconLinks.map((i)=>(
                                                <Link key={i.iconImageAlt} href={i.iconLink}>
                                                    <img loading="lazy" src={i.iconImageSrc} alt={i.iconImageAlt} height={27} width={27} className="w-[20px] h-[20px] dark:hidden hover:scale-[1.1] transition-transform ease-in-out"/>
                                                    <img loading="lazy" src={i.darkmodeSrc} alt={i.iconImageAlt} height={27} width={27} className="w-[20px] h-[20px] hidden dark:block hover:scale-[1.1] transition-transform ease-in-out"/>
                                                </Link>
                                            ))}
                                        </div>
                                    </div>
